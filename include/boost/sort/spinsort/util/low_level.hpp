...
/// @remarks
//-----------------------------------------------------------------------------
#ifndef __BOOST_SORT_SPINSORT_UTIL_LOW_LEVEL_HPP
#define __BOOST_SORT_SPINSORT_UTIL_LOW_LEVEL_HPP
...
inline Iter2_t full_merge (Iter1_t buf1, const Iter1_t end_buf1, Iter1_t buf2,
                    const Iter1_t end_buf2, Iter2_t buf_out, Compare comp)
{
    ...
    //-----------------------------------------------------------------------
/*
    if ( ((end_buf1 -buf1) + (end_buf2 - buf2)) > 4096 )
    {
    	if (buf1 == end_buf1) return init_move ( buf_out, buf2, end_buf2);
    	if (buf2 == end_buf2) return init_move ( buf_out, buf1, end_buf1);

    	if ( comp (*(end_buf1 -1), *buf2))
		{	Iter2_t itaux = init_move ( buf_out, buf1, end_buf1);
			return init_move ( itaux, buf2, end_buf2);
		};
		if ( comp (*(end_buf2 -1), *buf1 ))
		{	Iter2_t itaux = init_move(buf_out, buf2, end_buf2);
			return init_move ( itaux, buf1, end_buf1);
		};
    };
*/
...
inline Value_t *uninit_full_merge (Iter_t first1, const Iter_t last1, Iter_t first2,
...
//---------------------------------------------------------------------------
template < class Iter1_t, class Iter2_t, class Compare >
inline Iter2_t half_merge (Iter1_t buf1, const Iter1_t end_buf1, Iter2_t buf2,
                    const Iter2_t end_buf2, Iter2_t buf_out, Compare comp)
{
    ...
    //-----------------------------------------------------------------------
/*
    assert ((end_buf1 - buf1) == (buf2 - buf_out));
    if ((end_buf2 - buf_out) > 4096 )
    {
