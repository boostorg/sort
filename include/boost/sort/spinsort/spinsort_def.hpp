...
#include <boost/sort/parallel/detail/select_block_spreadsort.hpp>
#include <boost/sort/parallel/detail/parallel_stable_sort.hpp>
#include <boost/sort/parallel/detail/util/compare_traits.hpp>
#include <boost/sort/parallel/detail/sample_introsort.hpp>
#include <boost/sort/parallel/detail/sample_spreadsort.hpp>
#include <iterator>
...
//                                                                          ##
//                                                                          ##
//                       S A M P L E _ I N T R O S O R T                    ##
//                                                                          ##
//                                                                          ##
//############################################################################
//
//-----------------------------------------------------------------------------
//  function : sample_introsort
/// @brief parallel sample sort  algorithm (stable sort)
///
/// @param first : iterator to the first element of the range to sort
/// @param last : iterator after the last element to the range to sort
//-----------------------------------------------------------------------------
template < class Iter_t >
void sample_introsort (Iter_t first, Iter_t last)
{
    typedef compare_iter< Iter_t > Compare;
    detail::sample_introsort< Iter_t, Compare > (first, last);
};
//
//-----------------------------------------------------------------------------
//  function : sample_introsort
...
//-----------------------------------------------------------------------------
template < class Iter_t >
void sample_introsort (Iter_t first, Iter_t last, uint32_t nthread)
{
    typedef compare_iter< Iter_t > Compare;
    detail::sample_introsort< Iter_t, Compare > (first, last, nthread);
};
//
//-----------------------------------------------------------------------------
//  function : sample_introsort
...
template < class Iter_t, class Compare,
           enable_if_not_integral< Compare > * = nullptr >
void sample_introsort (Iter_t first, Iter_t last, Compare comp)
{
    detail::sample_introsort< Iter_t, Compare > (first, last, comp);
};
//
//-----------------------------------------------------------------------------
//  function : sample_introsort
...
//-----------------------------------------------------------------------------
template < class Iter_t, class Compare >
void sample_introsort (Iter_t first, Iter_t last, Compare comp, uint32_t nthread)
{
    detail::sample_introsort< Iter_t, Compare > (first, last, comp, nthread);
};
//
//############################################################################
//                                                                          ##
//                                                                          ##
//                       S A M P L E _ S P R E A D S O R T                  ##
//                                                                          ##
//                                                                          ##
//############################################################################
//
//-----------------------------------------------------------------------------
//  function : sample_spreadsort
/// @brief parallel sample sort  algorithm (stable sort)
///
/// @param first : iterator to the first element of the range to sort
/// @param last : iterator after the last element to the range to sort
//-----------------------------------------------------------------------------
template < class Iter_t >
void sample_spreadsort (Iter_t first, Iter_t last)
{
    typedef compare_iter< Iter_t > Compare;
    detail::sample_spreadsort< Iter_t, Compare > (first, last);
};
//
//-----------------------------------------------------------------------------
//  function : sample_spreadsort
...
//-----------------------------------------------------------------------------
template < class Iter_t >
void sample_spreadsort (Iter_t first, Iter_t last, uint32_t nthread)
{
    typedef compare_iter< Iter_t > Compare;
    detail::sample_spreadsort< Iter_t, Compare > (first, last, nthread);
};
//
//-----------------------------------------------------------------------------
//  function : sample_spreadsort
...
template < class Iter_t, class Compare,
           enable_if_not_integral< Compare > * = nullptr >
void sample_spreadsort (Iter_t first, Iter_t last, Compare comp)
{
    detail::sample_spreadsort< Iter_t, Compare > (first, last, comp);
};
//
//-----------------------------------------------------------------------------
//  function : sample_spreadsort
...
//-----------------------------------------------------------------------------
template < class Iter_t, class Compare >
void sample_spreadsort (Iter_t first, Iter_t last, Compare comp, uint32_t nthread)
{
    detail::sample_spreadsort< Iter_t, Compare > (first, last, comp, nthread);
};
//
//############################################################################
//                                                                          ##
//                                                                          ##
//                P A R A L L E L _ S P R E A D S O R T                     ##
//                                                                          ##
//                                                                          ##
//############################################################################
//
...
//-----------------------------------------------------------------------------
template < class Iter_t >
void parallel_spreadsort (Iter_t first, Iter_t last)
{
    typedef compare_iter< Iter_t > Compare;
    detail::select_block_indirect (first, last, Compare());
};
...
//-----------------------------------------------------------------------------
template < class Iter_t >
void parallel_spreadsort (Iter_t first, Iter_t last, uint32_t nthread)
{
    typedef compare_iter< Iter_t > Compare;
    detail::select_block_indirect (first, last, Compare(), nthread);
};
...
//-----------------------------------------------------------------------------
template < class Iter_t, class Compare >
void parallel_spreadsort (Iter_t first, Iter_t last, Compare comp, uint32_t nthread)
{
    detail::select_block_indirect (first, last, comp, nthread);
};
//
...
template < class Iter_t, class Compare,
           enable_if_not_integral< Compare > * = nullptr >
void parallel_spreadsort (Iter_t first, Iter_t last, Compare comp)
{
    detail::select_block_indirect (first, last, comp);
};
//
